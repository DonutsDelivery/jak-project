...
20. **`label_types.jsonc` entries crash the decompiler at load time if they reference types that are inside #| |# blocks in all-types.gc.** Symptom: rc=134 during `Error parsing labels for <file>: Type Error: Type <X> is unknown` — blocks ALL downstream files that import the loaded types (300+ file truncated scan). Cause: label_types_copy_queue.md lists jak3's type annotations as "confirmed copy-port" if the LABEL exists in jakx's binary, but doesn't verify that the TYPE ITSELF is active in jakx's all-types.gc. Fix before copy-port: for each entry, grep the type name against all-types.gc and skip rows where the type is commented/absent. Seen in: 14b308c3e tie-methods batch referenced inactive `tie-init-data`, blocked 324 files; reverted surgically in ea5b1d8a4 (kept 13/14 safe entries).
