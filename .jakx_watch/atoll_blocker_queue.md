...
| 2 | main.gc / display-loop | No entity-birth call in frame loop — actors-update wired but *spawn-actors* stayed #f | DONE @ next commit | PAIR3-B | actors-update wired in drawable.gc. *spawn-actors* now defined #t in jakx-stubs-late.gc AND re-asserted each frame in drawable.gc (counters game-info's set-#f during initialize!-'play). Game stable w/ fresh build; no crash. Next: verify actors birthed (need entity records in atoll BSP). |
