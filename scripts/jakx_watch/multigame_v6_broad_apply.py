...
def index_all_types(text: str):
    ...
    for i, line in enumerate(lines):
        ...
        # Track (:methods ... ) block start/end via paren depth.
        if "(:methods" in stripped and methods_open_line is None:
            in_methods = True
            methods_open_line = i
        ...
        if methods_open_line is not None and (i > methods_open_line) and in_methods and stripped.startswith("("):
            mm = RE_METHOD_LINE.match(line)
            if mm:
                mname = mm.group(2)
                idx = int(mm.group(5)) if mm.group(5) else None
                if idx is not None:
...
def classify_candidate(m, types_idx, ctx=None):
    ...
    # Tier classification: tier-A = single-corpus and "more than one corpus
    # would have hit but agree"; tier-B = single-corpus only.
    ...
    if "signature_diff" in cand:
        ...
        if kind == "extern":
            ...
            if existing is None:
                ...
                # Allow new idx within (cur_max, mca_val + max_method_add_gap].
                # Default gap = 4 (small step beyond current count).
                max_gap = ctx.get("method_add_max_gap", 4)
                upper = max(cur_max, mca_val - 1) + max_gap
                if jakx_method_idx > upper or jakx_method_idx < 9:
                    return {"action": "skip",
                            "reason": f"method-add-idx-out-of-range:idx={jakx_method_idx}/cur_max={cur_max}/mca={mca_val}",
                            "plan": None}
                ...
                # Compute new method-count-assert. Need at least jakx_method_idx + 1.
                new_mca = max(mca_val, jakx_method_idx + 1)
...
def main():
    ...
    # Rank: prioritize multi-corpus tier-A, then highest-error files, then n_instr
    # Build a quick lookup back to match for ranking
    rank_lookup = {(m["jakx_fn"], m["jakx_file"]): m for m in matches}
    def rank_key(p):
        m = rank_lookup[(p["jakx_fn"], p["jakx_file"])]
        return (
            0 if p["tier"] == "tier-A" else 1,
    ...
    for n, plan in enumerate(plans_to_apply, 1):
        ...
        msg_lines = [
            f"cand: {plan['jakx_fn']} @ {plan['jakx_file']} <- {plan['src_corpus']} ({plan['tier']})",
            "",
            f"v3-broad-apply candidate #{n}",
            f"actions: {info}",
        ]
        commit_msg = "\n".join(msg_lines)
        r = subprocess.run(
