#!/usr/bin/env python3
"""V8 broad-apply driver: V7 + REF-validated boost + jakx_functions corroboration.

V8 additions over V7:
  - REF-validated boost: when source fn appears in test/decompiler/reference/
    {jak1,jak2,jak3}/**/*_REF.gc (a passing offline-test reference file), add
    +0.5 to score. This is the strongest ground-truth signal: REF files are CI-
    enforced byte-identical to current decomp output.
  - jakx_functions corroboration: when jakx_fn name appears in
    game/mips2c/jakx_functions/*.cpp gLinkedFunctionTable.reg(...) entries,
    add +0.3 (smaller boost — it confirms name exists, not signature).
  - Both boosts loaded from .jakx_watch/research/{ref_validated_fns.json,
    extern_hints_from_jakx_functions.json} written by build_ref_validated_set.py
    and build_jakx_extern_hints.py.

Score recap (V7 + V8):
  +2.0  tier-A (multi-corpus)
  +0.5  jakx file has errors
  +0.5  source corpus = jak2
  +0.3  source corpus = jak3
  +0.1  source corpus = jak1
  +0.5  sig.kind = "extern"
  +0.5  V8: source fn is REF-validated
  +0.3  V8: jakx_fn name in jakx_functions hand-port set

V6/V7 features inherited unchanged below.
...
RE_DEFINE_EXTERN_NAME = re.compile(r"^\(define-extern\s+(\S+)")  # matches multi-line defs too
...
def index_all_types(text: str):
    ...
    externs = {}
    extern_names: set = set()  # all define-extern names, including multi-line forms
    deftypes = {}
    methods = {}
    last_extern_line = -1
    cur_type = None
    ...
    for i, line in enumerate(lines):
        ...
        if m and cur_type is None:
            ...
            last_extern_line = i
            continue
        # Also capture multi-line define-extern (name only, no full-sig on same line)
        mn = RE_DEFINE_EXTERN_NAME.match(line)
        if mn and cur_type is None:
            extern_names.add(mn.group(1))
    ...
    return {"externs": externs, "extern_names": extern_names, "deftypes": deftypes,
            "methods": methods, "lines": lines, "last_extern_line": last_extern_line}
...
def classify_candidate(m, types_idx, ctx=None):
    ...
    # V8: REF-validated source boost (+0.5)
    # Source fn appears in test/decompiler/reference/<corpus>/**/*_REF.gc — strong
    # ground-truth signal: the bytes are CI-enforced byte-identical to current decomp.
    ref_set = ctx.get("ref_validated", {}).get(src_corpus, set())
    src_fn = (m.get("candidates_by_corpus", {}).get(src_corpus) or [{}])[0].get("fn", "")
    if ref_set and src_fn and src_fn in ref_set:
        score += 0.5
        plan["ref_validated"] = True
    else:
        plan["ref_validated"] = False

    # V8: jakx_functions corroboration (+0.3)
    # jakx_fn name appears as a working hand-port in game/mips2c/jakx_functions —
    # smaller boost (confirms name exists, doesn't validate signature).
    extern_hints = ctx.get("extern_hints", set())
    if extern_hints and fn_name in extern_hints:
        score += 0.3
        plan["jakx_function_corroborated"] = True
    else:
        plan["jakx_function_corroborated"] = False
    ...
    if "signature_diff" in cand:
        ...
        if kind == "extern":
            ...
            # Also block extern-add for multi-line define-externs (not in externs dict
            # because RE_DEFINE_EXTERN requires a complete single-line form, but the
            # name is still defined — adding a second line would be a redefinition).
            if jakx_extern_idx is None and fn_name in types_idx.get("extern_names", set()):
                return {"action": "skip", "reason": "extern-add-multiline-exists", "plan": None}
...
def has_fwd_ref(new_line: str, line_idx: int, types_idx):
    ...
    # Skip first token (the keyword, e.g. "define-extern" or the method name).
    # For define-extern lines, also skip the second token (the function/var name
    # being declared) — it's not a type reference.
    skip_n = 2 if tokens and tokens[0] == "define-extern" else 1
    for tok in tokens[skip_n:] if len(tokens) > skip_n else []:
        if tok in BUILTINS:
            continue
...
def main():
    ...
    # V8: REF-validated source set
    ref_validated = {}
    ref_path = ROOT / ".jakx_watch" / "research" / "ref_validated_fns.json"
    if ref_path.exists():
        ref_data = json.loads(ref_path.read_text())
        ref_validated = {g: set(names) for g, names in ref_data.items()}
        sizes = ", ".join(f"{g}={len(s)}" for g, s in ref_validated.items())
        print(f"[{args.cycle_tag}] REF-validated set loaded ({sizes})", file=sys.stderr)
    else:
        print(f"[{args.cycle_tag}] WARN: ref_validated_fns.json missing — boost disabled", file=sys.stderr)

    # V8: jakx_functions extern hint set
    extern_hints = set()
    eh_path = ROOT / ".jakx_watch" / "research" / "extern_hints_from_jakx_functions.json"
    if eh_path.exists():
        eh_data = json.loads(eh_path.read_text())
        extern_hints = set(eh_data.keys())
        print(f"[{args.cycle_tag}] jakx_functions hint set loaded ({len(extern_hints)} entries)", file=sys.stderr)
    else:
        print(f"[{args.cycle_tag}] WARN: extern_hints_from_jakx_functions.json missing — boost disabled", file=sys.stderr)
    ...
    ctx = {
        ...
        "ref_validated": ref_validated,
        "extern_hints": extern_hints,
    }
