...
// (method 100 v-leopard) -- writes 7 stride-80 control vectors into a struct
...
// v-wolf method 100; L-labels and floats differ. Wrapped Tier 2 mips2c.
//
// L95-L101 vector data (from wcar-leopard_ir2.asm L95..L101):
//   L95  (0.0,       5939.2, -10649.6,  8192.0)   -> v1+524
//   L96  (0.0,       6144.0,   1638.4,  8192.0)   -> v1+444
//   L97  (0.0,       3276.8,  17408.0,  5734.4)   -> v1+364
//   L98  (-12288.0,  1843.2, -10854.4,  4505.6)   -> v1+284
//   L99  ( 12288.0,  1843.2, -10854.4,  4505.6)   -> v1+204
//   L100 (-12288.0,  2048.0,  16588.8,  4505.6)   -> v1+124
//   L101 ( 12288.0,  2048.0,  16588.8,  4505.6)   -> v1+44
namespace method_100_v_leopard {
struct Cache {
...
u64 execute(void* ctxt) {
  ...
  // L101 -> v1+44  (12288.0, 2048.0, 16588.8, 4505.6)
  c->lui(a2, 0x4640); c->ori(a2, a2, 0x0000); c->sw(a2, 44, v1);
  c->lui(a2, 0x4500); c->ori(a2, a2, 0x0000); c->sw(a2, 48, v1);
  c->lui(a2, 0x4681); c->ori(a2, a2, 0x999a); c->sw(a2, 52, v1);
  c->lui(a2, 0x458c); c->ori(a2, a2, 0xcccd); c->sw(a2, 56, v1);
  // L100 -> v1+124 (-12288.0, 2048.0, 16588.8, 4505.6)
  c->lui(a2, 0xc640); c->ori(a2, a2, 0x0000); c->sw(a2, 124, v1);
  c->lui(a2, 0x4500); c->ori(a2, a2, 0x0000); c->sw(a2, 128, v1);
  c->lui(a2, 0x4681); c->ori(a2, a2, 0x999a); c->sw(a2, 132, v1);
  c->lui(a2, 0x458c); c->ori(a2, a2, 0xcccd); c->sw(a2, 136, v1);
  // L99 -> v1+204 (12288.0, 1843.2, -10854.4, 4505.6)
  c->lui(a2, 0x4640); c->ori(a2, a2, 0x0000); c->sw(a2, 204, v1);
  c->lui(a2, 0x44e6); c->ori(a2, a2, 0x6666); c->sw(a2, 208, v1);
  c->lui(a2, 0xc629); c->ori(a2, a2, 0x999a); c->sw(a2, 212, v1);
  c->lui(a2, 0x458c); c->ori(a2, a2, 0xcccd); c->sw(a2, 216, v1);
  // L98 -> v1+284 (-12288.0, 1843.2, -10854.4, 4505.6)
  c->lui(a2, 0xc640); c->ori(a2, a2, 0x0000); c->sw(a2, 284, v1);
  c->lui(a2, 0x44e6); c->ori(a2, a2, 0x6666); c->sw(a2, 288, v1);
  c->lui(a2, 0xc629); c->ori(a2, a2, 0x999a); c->sw(a2, 292, v1);
  c->lui(a2, 0x458c); c->ori(a2, a2, 0xcccd); c->sw(a2, 296, v1);
  // L97 -> v1+364 (0.0, 3276.8, 17408.0, 5734.4)
  c->sw(r0, 364, v1);
  c->lui(a2, 0x454c); c->ori(a2, a2, 0xcccd); c->sw(a2, 368, v1);
  c->lui(a2, 0x4688); c->ori(a2, a2, 0x0000); c->sw(a2, 372, v1);
  c->lui(a2, 0x45b3); c->ori(a2, a2, 0x3333); c->sw(a2, 376, v1);
  // L96 -> v1+444 (0.0, 6144.0, 1638.4, 8192.0)
  c->sw(r0, 444, v1);
  ...
  // L95 -> v1+524 (0.0, 5939.2, -10649.6, 8192.0)
  c->sw(r0, 524, v1);
  c->lui(a2, 0x45b9); c->ori(a2, a2, 0x999a); c->sw(a2, 528, v1);
  c->lui(a2, 0xc626); c->ori(a2, a2, 0x6666); c->sw(a2, 532, v1);
  c->lui(a2, 0x4600); c->ori(a2, a2, 0x0000); c->sw(a2, 536, v1);
...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 100 v-leopard)", execute, 16);
}

} // namespace method_100_v_leopard

// (method 115 v-leopard) -- 4-iteration shock-joint quaternion updater.
// Same shape as v-cougar / v-wolf method 115 (no axis-angle, no mul). Pre-loop
// builds steering quat with +4096.0 multiplier (same sign as v-bobcat / v-wolf;
// v-cougar uses -4096.0). Stack 176 bytes.
namespace method_115_v_leopard {
struct Cache {
  void* sin;
  void* cos;
...
void link() {
  ...
  gLinkedFunctionTable.reg("(method 115 v-leopard)", execute, 176);
}
